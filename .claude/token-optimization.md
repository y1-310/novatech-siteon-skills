...
## Phase 1.5: Notion クエリ最適化（2026-04-20 完了）

### ゴール

標準タスク取得を 40,000 tokens → 3,000 tokens 以下に削減。

### Property ID 確定（NovaTech タスク一覧 DB）
...
> ⚠️「期限」カラムはスキーマに存在しない（確認済み）

### クエリパターン実測結果

| パターン | 説明 | page_size | filter_properties | 実測出力 | 推定tokens | 削減率 |
|---------|------|-----------|-------------------|---------|-----------|--------|
| **Pattern A** | 高優先度のみ | 10 | title/ステータス/優先度 | ~8,710 chars | **~2,177** | **▲95%** |
| **Pattern B** | 高+中優先度 | 20 | +カテゴリ | ~17,420 chars | **~4,355** | **▲89%** |
| **Pattern C** | 全件（棚卸し専用） | 100 | 全カラム | 160,776 chars | ~40,194 | ベースライン |

### Before / After 比較

| 指標 | Before（Pattern C） | After（Pattern A） | 削減 |
|------|--------------------|--------------------|------|
| 出力文字数 | 160,776 chars | ~8,710 chars | **▲94.6%** |
| 推定tokens | ~40,194 | ~2,177 | **▲94.6%** |
| 取得件数 | 77件全件 | 高優先度のみ（≤10件） | — |
| ファイル書き出し必要 | あり（上限超過） | なし（インライン表示可） | — |

### 月間節約試算

| シナリオ | 頻度 | 月間tokens（Before） | 月間tokens（After） | 月間節約 |
|---------|------|---------------------|--------------------|---------| 
| 手動確認 | 1回/日 | ~1,205,820 | ~65,310 | **▲1,140,510** |
| 秘書室自動 | 1回/日 | ~1,205,820 | ~65,310 | **▲1,140,510** |

### 成果物

- `.claude/notion-query-rules.md`（新規作成）— Pattern A/B/C の完全仕様、フローチャート、秘書室想定

### 残課題

- CLAUDE.md への統合: cc-company 秘書室アーキテクチャ確定後（Phase 3 と同時）
