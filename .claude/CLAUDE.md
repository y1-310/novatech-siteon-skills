...
| 制作・品質部長 | Opus 4.6 サブ → Claude Code記述 + Codex監査 | コード記述（Claude Code）・大量テキスト出力（Kimi）・品質チェック（Codex監査） |
...
CEO (Claude Code Opus 4.7) = 振り分け・最終判断・監督
  └─ Task ツールで部署長サブ起動 (Opus 4.6)
     ...
     ├─ 制作・品質部長    → Claude Code がコード記述 / Codex は監査専任 / Kimi はテキスト出力
...
- エラーの原因を分析する（分析）
- 顧客ステータスを管理する（管理）
- コード品質ゲート: **サイト完成単位**で Codex 監査を1回実行し、クロスチェックする（commit毎の監査は不要。重要度「高」の指摘があった場合のみ再監査。レポート形式は `.claude/rules.md` カテゴリ1）
...
| 制作・品質部長 | HTML/CSS/JS生成・コンセプト文・見積書・SEOテキスト等全出力 | Claude Code記述 + Codex監査 / Kimi |
