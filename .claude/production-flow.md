# cc-company 生産フロー v1.1（2026-09-07 役割反転を反映）
...
- Kimiは道具(判断しない)
- **Codexは監査専任**。コードの記述・修正は行わない（2026-09-07 役割反転）
- **HTML/CSS/JS の記述は Claude Code が行う**
- 記述以外の専門タスク（日本語コピー・リサーチ・画像生成）は専門AIへ自動委託
...
| タスク種別 | 実行者 | トリガー条件 |
|---|---|---|
| HTML/CSS/JS 新規生成 | **Claude Code** | skill準拠のLP/コンポーネント制作時（2026-09-07〜） |
| HTML/CSS/JS 修正（規模不問） | **Claude Code** | 監査指摘対応・緊急修正・細部調整 |
| コード監査 | **Codex** | サイト完成単位で1回。表形式レポートを返す |
| 画像生成 | **Codex (cx-image)** | SNS投稿画像・サイト用画像（反転対象外） |
...
### フェーズ4: 品質監督【制作・品質部署長sub-agent + Codex監査】

制作・品質部署長 sub-agent による自動レビュー:
...
2. レスポンシブ規則チェック（カテゴリ4の 40〜49 を含む）
3. アクセシビリティ・SEO 基本項目チェック

レポートは「指摘項目 / 該当ファイル・行 / 違反ルール番号 / 重要度(高・中・低)」の表形式。
**指摘の修正は Claude Code が実施する。再監査は重要度「高」の指摘があった場合のみ。**
...
タスク内容を分析
  ↓
┌─ HTML/CSS/JS生成含む? ─Yes→ Claude Code が記述 → 完成単位でCodex監査
│                         No
│                         ↓
...
### Codex 監査発注基準（2026-09-07 役割反転。委託基準から監査基準へ）

以下はすべて **Claude Code が記述する**:
- 新規HTMLファイル作成
- CSSの新規スタイル定義(行数不問)
- JavaScriptの新規ロジック実装
- lp-novatech skill準拠の LP 生成
- コンポーネント新規作成
- 既存コードの修正・文字列置換・コメント追加・ファイル構造整理

**Codex に監査を出すタイミング**:
- 1サイトの制作が完了した時点で1回（commit 毎には出さない）
- 重要度「高」の指摘を修正した後、その1回だけ再監査

**Codex に出さないもの**: コードの記述・修正依頼。指摘を受けて直すのは Claude Code の仕事。
...
### 緊急対応
Kimi障害時は Claude Code が代替実行可。
ただし事後報告必須、Notionに「例外実行」として記録。
Codex障害時は監査をスキップせず、復旧後に必ず実施する（未監査のままデプロイしない）。
...
"このタスクは本来 [Kimi/sub-agent] に委託すべきです。
 委託用プロンプトを作成しますか、このまま直接実行しますか?"

※ コード記述は 2026-09-07 以降 Claude Code の担当であり、違反検知の対象外。
   代わりに「サイト完成時に Codex 監査を出したか」をセルフチェックする。
