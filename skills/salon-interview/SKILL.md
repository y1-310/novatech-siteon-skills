...
|------|------------------|
| 美容室 | （共通のみ） |
| バーバー | ヒーロー直下CTA / Master Barber紹介 / 施術の流れ（Grooming Flow） / パララックス演出 / バーバーツール紹介。※キッズ対応は除外 |
| ネイル | 定額デザインコース / 持ち込みデザイン案内 / オフ代・ケアメニュー |
| エステ | 施術フロー図 / 初回体験コースCTA / お悩み別メニュー / ビフォーアフター表示形式（写真並列 / スライダー比較 / おまかせ） |
| まつエク | デザイン比較（カール・本数） / 素材別説明 / 本数別イメージ写真 |

#### Q13 追加選択肢（nail-esthe判定時）
Q13のカラー方向性にnail-esthe用プリセットを追加表示：
- 既存：フェミニン×上品 / ラベンダー×洗練 / ベージュ×ナチュラル / おまかせ
- **追加：ラグジュアリーグリーン / ポップ / ミニマル**

#### Q16 追加選択肢（nail-esthe判定時）
- **追加：スプリットレイアウト（プロ以上）**

#### Q17 追加選択肢
- **追加：グリッド型（nail-estheではデフォルト）**

#### Q13 追加選択肢（barber判定時）
- **追加：ストリート / 和テイスト**

#### Q14後の補足質問（barber判定時）
- 「写真のトーン処理は？」（single_select: モノクロ / セピア / カラーのまま / おまかせ）→ `photo_filter_style`

#### Q18 追加選択肢（barber判定時）
- **共通追加：ヒーロー直下CTA（デフォルトON）/ Master Barber紹介 / 施術の流れ（Grooming Flow、プロ以上）/ パララックス演出（プロ以上）/ バーバーツール紹介**
- **除外：キッズ対応表示**

#### Q18 追加選択肢（salon-group判定時）
- **共通追加：ヒーロー直下CTA（デフォルトON）/ 代表メッセージ / エリア別店舗選択（デフォルトON）**
- 採用セクション選択時の追加質問：「採用専用ページを作成しますか？」（はい / いいえ）→ はいならrecruit.html生成（プロ以上）

#### Q18 追加選択肢（nail-esthe判定時・共通部分）
- **追加：ヒーロー直下CTA（デフォルトON） / 施術者紹介 / 施術の流れ（Treatment Flow、プロ以上、エステはデフォルトON）**
- エステ判定時のビフォーアフター選択時：「表示形式は？」（写真並列 / スライダー比較 / おまかせ）
- まつエク判定時：「デザイン比較（カール・本数）」をデフォルトON推奨
...
| 駐車場情報 | `parking` |
| 道順写真 | `directions` |
| 施術者紹介 | `therapist_intro` |
| 代表メッセージ | `ceo_message` |
| エリア別店舗選択 | `area_salon_select` |
| 採用専用ページ作成 | `recruit_page` (値: `true` / `false`) |
| Master Barber紹介 | `master_barber` |
| 施術の流れ（Grooming Flow） | `grooming_flow` |
| パララックス演出 | `parallax` |
| バーバーツール紹介 | `barber_tools` |
| 写真トーン処理 | `photo_filter_style` (値: `mono` / `sepia` / `color` / `auto`) |
| 施術の流れ（Treatment Flow） | `treatment_flow` |
| ビフォーアフター表示形式 | `before_after_style` (値: `side_by_side` / `slider` / `auto`) |
