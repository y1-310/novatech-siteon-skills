---
name: lp-barber
description: メンズバーバー・フェードカット専門店向けLPを生成する。salon-interviewで業態判定「バーバー」と確定した後にヒアリングJSONを受け取り、ダークトーン・力強いデザイン原則でプラン判定→サイト生成を実行する。女性向けlp-salonとはデザイン体系が全く異なる。
---
...
### オプションセクション（v1.18追加含む）

| セクション | 条件 | デフォルト |
|-----------|------|----------|
| ヒーロー直下CTA | Q18選択 | ON |
| Google口コミ | Q18選択 | OFF（数値バー/全文カード/両方） |
| Master Barber紹介 | Q18選択 | OFF |
| 施術の流れ（Grooming Flow） | Q18選択（プロ以上） | OFF |
| パララックス演出 | Q18選択（プロ以上） | OFF |
| バーバーツール紹介 | Q18選択 | OFF |
| 採用セクション | Q18選択 | OFF |
| FAQ | Q18選択 | OFF |
| 物販・商品紹介 | Q18選択 | OFF |

※ barber では「キッズ対応表示」を選択肢から除外
